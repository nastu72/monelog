...
if(!empty($_POST)){
    ...
    if(empty($err_msg)){
        ...
        try{
            ...
            // ②DB新規登録の場合($edit_flgがtrueの場合)
            }else{
                debug('DB新規登録！！');
                $sql = 'INSERT INTO expense(day,category_id,name,price,pic,user_id,create_date) VALUES (:day,:category,:name,:price,:pic,:u_id,:date)';
                $data = array(':day'=>$day,':category'=>$category,':name'=>$name,':price'=>$price,':pic'=>$pic,':u_id'=>$_SESSION['user_id'],':date'=>date('Y-m-d H:i:s'));
            }
            debug('SQL：'.$sql);
...
<?php 
$siteTitle = (!$edit_flg) ? '支出の新規登録' : '支出の編集';
require('head.php');
?>
...
<body class="page-1colum">
    ...
    <h2 class="title page-title"><?php echo (!$edit_flg) ? '【支出】いくら使った？' : '支出の編集'; ?></h2>
    <!-- Main -->
    <section id="main">
        <div class="form-container">
            <form action="" method="post" class="form" enctype="multipart/form-data">
                ...
                <div class="new-category">
                    <a href="expenseCategory.php">支出カテゴリの新規登録</a>
                </div>
                <!-- 内容フォーム -->
